        ...
        if infer:
            ...
            if self.use_shallow_diffusion:
                if self.train_aux_decoder:
                    aux_cond = (
                        condition * self.aux_decoder_grad
                        + condition.detach() * (1 - self.aux_decoder_grad)
                    )
...
class DiffSingerVariance(CategorizedModule, ParameterAdaptorModule):
    ...
    def __init__(self, vocab_size):
        ...
        assert self.use_stretch_embed is not None, (
            "You may be loading an old version of the model checkpoint, which is incompatible with the new version due to some bug fixes. It is recommended to roll back to the old version (commit id: 6df0ee977c3728f14cb79c2db8b19df30b23a0bf)"
        )
        if self.use_stretch_embed and (self.predict_pitch or self.predict_variances):
            ...
            self.stretch_embed_rnn = nn.GRU(
                hparams["hidden_size"], hparams["hidden_size"], 1, batch_first=True
            )
        ...
        if self.predict_pitch:
            ...
            if self.use_melody_encoder:
                self.melody_encoder = MelodyEncoder(
                    enc_hparams=hparams["melody_encoder_args"]
                )
            ...
            self.pitch_backbone_type = compat.get_backbone_type(
                hparams, nested_config=pitch_hparams
            )
            self.pitch_backbone_args = compat.get_backbone_args(
                pitch_hparams, backbone_type=self.pitch_backbone_type
            )
            if self.diffusion_type == "ddpm":
        ...
        if self.predict_variances:
            ...
            if self.diffusion_type == "ddpm":
                ...
                self.variance_predictor = self.build_adaptor(
                    cls=MultiVarianceRectifiedFlow
                )
            else:
                raise NotImplementedError(self.diffusion_type)
        ...
        **kwargs,
        ...
        if self.use_spk_id:
            ...
            if ph_spk_mix_embed is not None and spk_mix_embed is not None:
                ...
                ph_spk_embed = spk_embed = self.spk_embed(spk_id)[
                    :, None, :
                ]  # [B,] => [B, T=1, H]
        else:
        ...
        if self.use_key_shift_embed and key_shift is not None:
            ks = key_shift[:, :, None]
            ks *= (
                self.custom_variance_scaling_factor["key_shift"]
                if self.use_variance_scaling
                else self.default_variance_scaling_factor["key_shift"]
            )
            condition += self.key_shift_embed(ks)

        if self.use_speed_embed and speed is not None:
            sp = speed[:, :, None]
            sp *= (
                self.custom_variance_scaling_factor["speed"]
                if self.use_variance_scaling
                else self.default_variance_scaling_factor["speed"]
            )
            condition += self.speed_embed(sp)
        ...
        if self.predict_pitch:
            if self.use_melody_encoder:
                ...
                pitch_cond = (
                    condition.clone()
                )  # preserve the original tensor to avoid further inplace operations
            ...
            if pitch_expr is None:
                ...
                pitch_retake_embed = (
                    pitch_expr * retake_true_embed
                    + (1.0 - pitch_expr) * retake_false_embed
                )
            ...
            if self.use_melody_encoder:
                ...
                if self.use_variance_scaling:
                    pitch_cond += self.delta_pitch_embed(
                        delta_pitch_in[:, :, None] / 12
                    )
                else:
            ...
            if infer:
                ...
                pitch_pred_out = self.pitch_predictor(
                    pitch_cond, pitch - base_pitch, infer=False
                )
        else:
            pitch_pred_out = None
        ...
        if variance_retake is not None:
            variance_embeds = [
                self.variance_embeds[v_name](
                    v_input[:, :, None] * self.variance_retake_scaling[v_name]
                )
                * ~variance_retake[v_name][:, :, None]
                for v_name, v_input in zip(
                    self.variance_prediction_list, variance_inputs
                )
            ]
            var_cond += torch.stack(variance_embeds, dim=-1).sum(-1)

        variance_outputs = self.variance_predictor(
            var_cond, variance_inputs, infer=infer
        )
