...
class VarianceDataset(BaseDataset):
    def __init__(self, prefix, preload=False):
        super(VarianceDataset, self).__init__(
            prefix, hparams["dataset_size_key"], preload
        )
        ...
        self.predict_variances = (
            need_energy or need_breathiness or need_voicing or need_tension
        )
    ...
    def collater(self, samples):
        ...
        batch.update({"tokens": tokens, "ph_dur": ph_dur})
        ...
        if hparams["predict_pitch"]:
            ...
            batch["note_rest"] = utils.collate_nd(
                [s["note_rest"] for s in samples], True
            )
            batch["note_dur"] = utils.collate_nd([s["note_dur"] for s in samples], 0)
            if hparams["use_glide_embed"]:
                batch["note_glide"] = utils.collate_nd(
                    [s["note_glide"] for s in samples], 0
                )
            batch["mel2note"] = utils.collate_nd([s["mel2note"] for s in samples], 0)
            batch["base_pitch"] = utils.collate_nd(
                [s["base_pitch"] for s in samples], 0
            )
        ...
        if hparams["predict_breathiness"]:
            batch["breathiness"] = utils.collate_nd(
                [s["breathiness"] for s in samples], 0
            )
...
def random_retake_masks(b, t, device):
    ...
    B_masks = (
        torch.randint(low=0, high=4, size=(b, 1), dtype=torch.long, device=device) == 0
    )
...
class VarianceTask(BaseTask):
    ...
    def build_losses_and_metrics(self):
        if self.predict_dur:
            ...
            self.register_validation_metric(
                "rhythm_corr", RhythmCorrectness(tolerance=0.05)
            )
            self.register_validation_metric(
                "ph_dur_acc", PhonemeDurationAccuracy(tolerance=0.2)
            )
        if self.predict_pitch:
            ...
            self.register_validation_metric(
                "pitch_acc", RawCurveAccuracy(tolerance=0.5)
            )
    ...
    def run_model(self, sample, infer=False):
        ...
        output = self.model(
            ...
            voicing=voicing,
            tension=tension,
            pitch_retake=pitch_retake,
            variance_retake=variance_retake,
            spk_id=spk_ids,
            key_shift=key_shift,
            speed=speed,
            infer=infer,
        ...
        if infer:
            ...
            if dur_pred is not None:
                losses["dur_loss"] = self.lambda_dur_loss * self.dur_loss(
                    dur_pred, ph_dur, ph2word=ph2word
                )
    ...
    def _validation_step(self, sample, batch_idx):
        ...
        if min(sample["indices"]) < hparams["num_valid_plots"]:
            ...
            def sample_get(key, idx, abs_idx):
                return sample[key][idx][
                    : self.valid_dataset.metadata[key][abs_idx]
                ].unsqueeze(0)
                ...
                if data_idx < hparams["num_valid_plots"]:
                    if dur_preds is not None:
                        ...
                        self.plot_dur(
                            ...
                            gt_dur,
                            pred_dur,
                            txt=self.valid_dataset.metadata["ph_texts"][
                                data_idx
                            ].split(),
                        )
                    if pitch_preds is not None:
                        pitch_len = self.valid_dataset.metadata["pitch"][data_idx]
                        pred_pitch = sample_get(
                            "base_pitch", i, data_idx
                        ) + pitch_preds[i][:pitch_len].unsqueeze(0)
                        gt_pitch = sample_get("pitch", i, data_idx)
                        mask = (sample_get("mel2ph", i, data_idx) > 0) & ~sample_get(
                            "uv", i, data_idx
                        )
                        self.valid_metrics["pitch_acc"].update(
                            pred=pred_pitch, target=gt_pitch, mask=mask
                        )
                        self.valid_metrics["pitch_r2"].update(
                            pred=pred_pitch, target=gt_pitch, mask=mask
                        )
                        self.plot_pitch(
                            data_idx,
                    ...
                    for name in self.variance_prediction_list:
                        ...
                        pred_variances = variances_preds[name][i][
                            :variance_len
                        ].unsqueeze(0)
                        mask = (sample_get("mel2ph", i, data_idx) > 0) & ~sample_get(
                            "uv", i, data_idx
                        )
                        self.valid_metrics[f"{name}_r2"].update(
                            pred=pred_variances, target=gt_variances, mask=mask
                        )
                        self.plot_curve(
                            data_idx,
    ...
    def plot_dur(self, data_idx, gt_dur, pred_dur, txt=None):
        ...
        self.logger.all_rank_experiment.add_figure(
            f"dur_{data_idx}",
            dur_to_figure(gt_dur, pred_dur, txt, title_text),
            self.global_step,
        )

    def plot_pitch(
        self, data_idx, gt_pitch, pred_pitch, note_midi, note_dur, note_rest
    ):
