...
from augmentation.variance_stretch import VarianceStretchAugmentation
...
# Augmentation task type constants
_AUG_FROM_RAW = 0
_AUG_FROM_AUG = 1
_AUG_MUTATE = 2
...
class VarianceBinarizer(BaseBinarizer):
    def __init__(self):
        ...
        self.predict_variances = (
            predict_energy or predict_breathiness or predict_voicing or predict_tension
        )
    ...
    def load_meta_data(self, raw_data_dir: pathlib.Path, ds_id, spk, lang):
        ...
        with open(raw_data_dir / "transcriptions.csv", "r", encoding="utf8") as f:
            for utterance_label in csv.DictReader(f):
                ...
                item_idx = (
                    int(item_name.rsplit(DS_INDEX_SEP, maxsplit=1)[-1])
                    if DS_INDEX_SEP in item_name
                    else 0
                )
                ...
                def require(attr, optional=False):
                    ...
                    if value is None and not optional:
                        raise ValueError(
                            f"Missing required attribute {attr} of item '{item_name}'."
                        )
                    return value
                ...
                temp_dict = {
                    ...
                    "ph_seq": self.phoneme_dictionary.encode(
                        require("ph_seq"), lang=lang
                    ),
                ...
                if hparams["predict_pitch"]:
                    ...
                    temp_dict["note_dur"] = [
                        float(x) for x in require("note_dur").split()
                    ]
    ...
    def check_coverage(self):
        ...
        for i, key in enumerate(sorted(midi_map.keys())):
            ...
            print(
                f"'{librosa.midi_to_note(key, unicode=False)}': {midi_map[key]}",
                end=end,
            )
        ...
        notes = [
            librosa.midi_to_note(m, unicode=False)
            for m in range(midis[0], midis[-1] + 1)
        ]
        ...
        if self.use_glide_embed:
            ...
            glide_count = {g: 0 for g in self.glide_map}
            for item_name in self.items:
            ...
            for i, key in enumerate(
                sorted(glide_count.keys(), key=lambda k: self.glide_map[k])
            ):
    ...
    def process_item(self, item_name, meta_data, binarization_args):
        ...
        ph_acc = torch.round(
            torch.cumsum(ph_dur_sec, dim=0) / self.timestep + 0.5
        ).long()
        ph_dur = torch.diff(
            ...
            if f0_seq is not None:
                f0 = resample_align_curve(
                    ...
                    original_timestep=float(
                        self.load_attr_from_ds(
                            ds_id, name, "f0_timestep", idx=ds_seg_idx
                        )
                    ),
        ...
        pitch = torch.from_numpy(
            librosa.hz_to_midi(f0.astype(np.float32)).astype(np.float32)
        ).to(self.device)
        ...
        if hparams["predict_dur"]:
            ...
            mel2dur = torch.gather(
                F.pad(ph_dur, [1, 0], value=1), 0, mel2ph
            )  # frame-level phone duration
            ph_midi = pitch.new_zeros(T_ph + 1).scatter_add(0, mel2ph, pitch / mel2dur)[
                1:
            ]
            processed_input["midi"] = (
                ph_midi.round().long().clamp(min=0, max=127).cpu().numpy()
            )
        ...
        if hparams["predict_pitch"]:
            ...
            note_acc = torch.round(
                torch.cumsum(note_dur_sec, dim=0) / self.timestep + 0.5
            ).long()
            note_dur = torch.diff(
                note_acc, dim=0, prepend=torch.LongTensor([0]).to(self.device)
            )
            ...
            frame_midi_pitch = torch.gather(
                F.pad(note_midi, [1, 0], value=0), 0, mel2note
            )
            global midi_smooth
        ...
        if hparams["predict_energy"]:
            ...
            if self.prefer_ds:
                energy_seq = self.load_attr_from_ds(
                    ds_id, name, "energy", idx=ds_seg_idx
                )
            ...
            if energy_from_wav:
                ...
                energy = (
                    energy_smooth(torch.from_numpy(energy).to(self.device)[None])[0]
                    .cpu()
                    .numpy()
                )
        ...
        if hparams["predict_breathiness"]:
            ...
            if self.prefer_ds:
                breathiness_seq = self.load_attr_from_ds(
                    ds_id, name, "breathiness", idx=ds_seg_idx
                )
            ...
            if breathiness_from_wav:
                ...
                breathiness = (
                    breathiness_smooth(
                        torch.from_numpy(breathiness).to(self.device)[None]
                    )[0]
                    .cpu()
                    .numpy()
                )
        ...
        if hparams["predict_voicing"]:
            ...
            if self.prefer_ds:
                voicing_seq = self.load_attr_from_ds(
                    ds_id, name, "voicing", idx=ds_seg_idx
                )
            ...
            if voicing_from_wav:
                ...
                voicing = (
                    voicing_smooth(torch.from_numpy(voicing).to(self.device)[None])[0]
                    .cpu()
                    .numpy()
                )
        ...
        if hparams["predict_tension"]:
            ...
            if self.prefer_ds:
                tension_seq = self.load_attr_from_ds(
                    ds_id, name, "tension", idx=ds_seg_idx
                )
            ...
            if tension_from_wav:
                ...
                tension = (
                    tension_smooth(torch.from_numpy(tension).to(self.device)[None])[0]
                    .cpu()
                    .numpy()
                )
    ...
    def arrange_data_augmentation(self, data_iterator):
        ...
        if self.augmentation_args["random_pitch_shifting"]["enabled"]:
            ...
            aug_ins = VarianceStretchAugmentation(
                self.raw_data_dirs, aug_args, pe=aug_pe
            )
            scale = aug_args["scale"]
            aug_item_names = random.choices(
                all_item_names, k=int(scale * len(all_item_names))
            )
        ...
        if self.augmentation_args["fixed_pitch_shifting"]["enabled"]:
            ...
            assert hparams["use_spk_id"], (
                "Fixed pitch shifting augmentation requires use_spk_id == True."
            )
            ...
            for i, target in enumerate(targets):
                ...
                for aug_item_name in aug_item_names:
                    replace_spk_id = (
                        self.spk_ids[int(aug_item_name.split(":", maxsplit=1)[0])]
                        + (i + 1) * spk_id_size
                    )
                    aug_task = {
                        "name": aug_item_name,
        ...
        if self.augmentation_args["random_time_stretching"]["enabled"]:
            ...
            k_from_aug = int(
                total_scale * scale / (1 + total_scale) * len(all_item_names)
            )
            k_mutate = int(total_scale * scale / (1 + scale) * len(all_item_names))
            aug_types = (
                [_AUG_FROM_RAW] * k_from_raw
                + [_AUG_FROM_AUG] * k_from_aug
                + [_AUG_MUTATE] * k_mutate
            )
            aug_items = random.choices(all_item_names, k=k_from_raw) + random.choices(
                aug_list, k=k_from_aug + k_mutate
            )

            for aug_type, aug_item in zip(aug_types, aug_items):
                speed = speed_min * (speed_max / speed_min) ** random.random()
                if aug_type == _AUG_FROM_RAW:
                    aug_task = {
                        "name": aug_item,
                        "func": aug_ins.process_item,
                        "kwargs": {"speed": speed},
