...
class DiffSingerVarianceExporter(BaseExporter):
    def __init__(
        ...
        self.use_lang_id = (
            hparams.get("use_lang_id", False)
            and len(self.phoneme_dictionary.cross_lingual_phonemes) > 0
        )
        ...
        self.pitch_backbone_class_name = (
            remove_suffix(
                self.model.pitch_predictor.backbone.__class__.__name__, "ONNX"
            )
        ...
        self.variance_backbone_class_name = (
            remove_suffix(
                self.model.variance_predictor.backbone.__class__.__name__, "ONNX"
            )
        ...
        self.freeze_spk: Tuple[str, Dict[str, float]] = (
            freeze_spk if hparams["use_spk_id"] else None
        )
        self.export_spk: List[Tuple[str, Dict[str, float]]] = (
            export_spk if hparams["use_spk_id"] and export_spk is not None else []
        )
        if hparams["use_spk_id"]:
            if not self.export_spk and self.freeze_spk is None:
                ...
                if len(self.spk_map) == 1:
                    ...
                    self.export_spk = [
                        (name, {name: 1.0}) for name in self.spk_map.keys()
                    ]
            if self.freeze_spk is not None:
                self.model.register_buffer(
                    "frozen_spk_embed", self._perform_spk_mix(self.freeze_spk[1])
                )
        if hparams.get("use_key_shift_embed", False):
            self.model.register_buffer(
                "frozen_key_shift", torch.FloatTensor([0.0]).to(self.device)
            )
        if hparams.get("use_speed_embed", False):
            self.model.register_buffer(
                "frozen_speed", torch.FloatTensor([1.0]).to(self.device)
            )

    def build_model(self) -> DiffSingerVarianceONNX:
        model = (
            DiffSingerVarianceONNX(
    ...
    def export_model(self, path: Path, model_name: str = None):
        ...
        linguistic_onnx = self._optimize_linguistic_graph(
            onnx.load(self.linguistic_encoder_cache_path)
        )
        ...
        if self.model.predict_dur:
            dur_predictor_onnx = self._optimize_dur_predictor_graph(
                onnx.load(self.dur_predictor_cache_path)
            )
    ...
    def export_attachments(self, path: Path):
        for spk in self.export_spk:
            self._export_spk_embed(
                path / f"{self.model_name}.{spk[0]}.emb", self._perform_spk_mix(spk[1])
            )
        ...
        if len(self.export_spk) > 0:
            dsconfig["speakers"] = [
                f"{self.model_name}.{spk[0]}" for spk in self.export_spk
            ]
        # functionalities
        if self.model.predict_dur:
        ...
        if self.model.predict_variances:
            ...
            for variance in VARIANCE_CHECKLIST:
                dsconfig[f"predict_{variance}"] = (
                    variance in self.model.variance_prediction_list
                )
        # augmentation config
        if hparams.get("use_key_shift_embed", False):
            dsconfig["augmentation_args"] = {
                "random_pitch_shifting": {
                    "range": hparams["augmentation_args"]["random_pitch_shifting"][
                        "range"
                    ]
                }
            }
    ...
    def _torch_export_model(self):
        ...
        encoder_out = torch.rand(
            1, 5, hparams["hidden_size"], dtype=torch.float32, device=self.device
        )
        x_masks = tokens == 0
        ph_midi = torch.LongTensor([[60] * 5]).to(self.device)
        encoder_output_names = ["encoder_out", "x_masks"]
        encoder_common_axes = {
            "encoder_out": {1: "n_tokens"},
            "x_masks": {1: "n_tokens"},
        }
        ...
        if self.model.predict_dur:
            torch.onnx.export(
                ...
                (tokens, word_div, word_dur, *([languages] if input_lang_id else [])),
            ...
            torch.onnx.export(
                self.model.view_as_linguistic_encoder(),
                (tokens, ph_dur, *([languages] if input_lang_id else [])),
                self.linguistic_encoder_cache_path,
                input_names=[
                    "tokens",
                    "ph_dur",
        ...
        # Common dummy inputs
        dummy_time = (
            torch.rand((1,), device=self.device) * hparams.get("time_scale_factor", 1.0)
        ).float()
        dummy_steps = 5
        ...
        if self.model.predict_pitch:
            ...
            use_glide_embed = (
                use_melody_encoder
                and hparams["use_glide_embed"]
                and not self.freeze_glide
            )
            # Prepare inputs for preprocessor of the pitch predictor
            note_midi = torch.FloatTensor([[60.0] * 4]).to(self.device)
            note_dur = torch.LongTensor([[2, 6, 3, 4]]).to(self.device)
            pitch = torch.FloatTensor([[60.0] * 15]).to(self.device)
            ...
            pitch_input_args = (
                ...
                ph_dur,
                {
                    ...
                    **({"note_rest": note_midi >= 0} if use_melody_encoder else {}),
                    "note_dur": note_dur,
                    **(
                        {"note_glide": torch.zeros_like(note_midi, dtype=torch.long)}
                        if use_glide_embed
                        else {}
                    ),
                    "pitch": pitch,
            ...
            torch.onnx.export(
                ...
                output_names=["pitch_cond", "base_pitch"],
                dynamic_axes={
                    "encoder_out": {1: "n_tokens"},
                    "ph_dur": {1: "n_tokens"},
                    "note_midi": {1: "n_notes"},
                    ...
                    **({"expr": {1: "n_frames"}} if self.expose_expr else {}),
                    "retake": {1: "n_frames"},
                    "pitch_cond": {1: "n_frames"},
                    "base_pitch": {1: "n_frames"},
            ...
            torch.onnx.export(
                pitch_predictor,
                (condition.transpose(1, 2), dummy_steps),
                self.pitch_predictor_cache_path,
                input_names=["pitch_cond", "steps"],
                output_names=["x_pred"],
                dynamic_axes={"pitch_cond": {1: "n_frames"}, "x_pred": {1: "n_frames"}},
                opset_version=17,
                **onnx_helper.TORCHSCRIPT_EXPORT_KWARGS,
            )
            ...
            torch.onnx.export(
                ...
                input_names=["x_pred", "base_pitch"],
                output_names=["pitch_pred"],
                dynamic_axes={
                    "x_pred": {1: "n_frames"},
                    "base_pitch": {1: "n_frames"},
                    "pitch_pred": {1: "n_frames"},
                },
                opset_version=17,
                **onnx_helper.TORCHSCRIPT_EXPORT_KWARGS,
            )
        ...
        if self.model.predict_variances:
            total_repeat_bins = hparams["variances_prediction_args"][
                "total_repeat_bins"
            ]
            ...
            pitch = torch.FloatTensor([[60.0] * 15]).to(self.device)
            variances = {
                v_name: torch.FloatTensor([[0.0] * 15]).to(self.device)
                for v_name in self.model.variance_prediction_list
            }
            retake = torch.ones_like(pitch, dtype=torch.bool)[..., None].tile(
                len(self.model.variance_prediction_list)
            )
            torch.onnx.export(
            ...
            torch.onnx.export(
                ...
                input_names=["variance_cond", "steps"],
                output_names=["xs_pred"],
                dynamic_axes={
                    "variance_cond": {1: "n_frames"},
                    "xs_pred": {
                        (
                            1 if len(self.model.variance_prediction_list) == 1 else 2
                        ): "n_frames"
                    },
                },
                opset_version=17,
            ...
            torch.onnx.export(
                ...
                input_names=["xs_pred"],
                output_names=[
                    f"{v_name}_pred" for v_name in self.model.variance_prediction_list
                ],
                dynamic_axes={
                    "xs_pred": {
                        (
    ...
    def _perform_spk_mix(self, spk_mix: Dict[str, float]):
        ...
        spk_mix_value_N = torch.FloatTensor(spk_mix_values).to(self.device)[
            None
        ]  # => [1, N]
        spk_mix_value_sum = spk_mix_value_N.sum()
        assert spk_mix_value_sum > 0.0, (
            f"Speaker mix checks failed.\nProportions of speaker mix sum to zero."
        )
        spk_mix_value_N /= spk_mix_value_sum  # normalize
        spk_mix_embed = torch.sum(
            self.model.spk_embed(spk_mix_id_N)
            * spk_mix_value_N.unsqueeze(2),  # => [1, N, H]
            dim=1,
            keepdim=True,
        )  # => [1, 1, H]
        return spk_mix_embed

    def _optimize_linguistic_graph(
        self, linguistic: onnx.ModelProto
    ) -> onnx.ModelProto:
        onnx_helper.model_override_io_shapes(
            linguistic,
            output_shapes={"encoder_out": (1, "n_tokens", hparams["hidden_size"])},
        )
        ...
        onnx_helper.model_reorder_io_list(
            linguistic, "input", target_name="languages", insert_after_name="tokens"
        )
        print(f"| optimize graph: {self.fs2_class_name}")
        return linguistic

    def _optimize_dur_predictor_graph(
        self, dur_predictor: onnx.ModelProto
    ) -> onnx.ModelProto:
        onnx_helper.model_override_io_shapes(
            dur_predictor, output_shapes={"ph_dur_pred": (1, "n_tokens")}
        )
    ...
    def _optimize_merge_pitch_predictor_graph(
        self,
        pitch_pre: onnx.ModelProto,
        pitch_predictor: onnx.ModelProto,
        pitch_post: onnx.ModelProto,
    ) -> onnx.ModelProto:
        onnx_helper.model_override_io_shapes(
            pitch_pre,
            output_shapes={"pitch_cond": (1, "n_frames", hparams["hidden_size"])},
        )
        ...
        onnx_helper.model_add_prefixes(
            pitch_pre, node_prefix="/pre", ignored_pattern=r".*embed.*"
        )
        onnx_helper.model_add_prefixes(
            pitch_pre,
            dim_prefix="pre.",
            ignored_pattern="(n_tokens)|(n_notes)|(n_frames)",
        )
        onnx_helper.model_add_prefixes(
            pitch_post, node_prefix="/post", ignored_pattern=None
        )
        onnx_helper.model_add_prefixes(
            pitch_post, dim_prefix="post.", ignored_pattern="n_frames"
        )
        pitch_pre_diffusion = onnx.compose.merge_models(
            pitch_pre,
            pitch_predictor,
            io_map=[("pitch_cond", "pitch_cond")],
            prefix1="",
            prefix2="",
            doc_string="",
            producer_name=pitch_pre.producer_name,
            producer_version=pitch_pre.producer_version,
            domain=pitch_pre.domain,
            model_version=pitch_pre.model_version,
        )
        pitch_pre_diffusion.graph.name = pitch_pre.graph.name
        pitch_predictor = onnx.compose.merge_models(
            pitch_pre_diffusion,
            pitch_post,
            io_map=[("x_pred", "x_pred"), ("base_pitch", "base_pitch")],
            prefix1="",
            prefix2="",
            doc_string="",
            producer_name=pitch_pre.producer_name,
    ...
    def _optimize_merge_variance_predictor_graph(
        self,
        var_pre: onnx.ModelProto,
        var_diffusion: onnx.ModelProto,
        var_post: onnx.ModelProto,
    ):
        onnx_helper.model_override_io_shapes(
            var_pre,
            output_shapes={"variance_cond": (1, "n_frames", hparams["hidden_size"])},
        )
        ...
        ignored_variance_names = "|".join(
            [f"({v_name})" for v_name in self.model.variance_prediction_list]
        )
        onnx_helper.model_add_prefixes(
            var_pre,
            node_prefix="/pre",
            value_info_prefix="/pre",
            initializer_prefix="/pre",
            ignored_pattern=rf".*((embed)|{ignored_variance_names}).*",
        )
        onnx_helper.model_add_prefixes(
            var_pre, dim_prefix="pre.", ignored_pattern="(n_tokens)|(n_frames)"
        )
        onnx_helper.model_add_prefixes(
            var_post,
            node_prefix="/post",
            value_info_prefix="/post",
            initializer_prefix="/post",
            ignored_pattern=None,
        )
        onnx_helper.model_add_prefixes(
            var_post, dim_prefix="post.", ignored_pattern="n_frames"
        )
        ...
        var_pre_diffusion = onnx.compose.merge_models(
            var_pre,
            var_diffusion,
            io_map=[("variance_cond", "variance_cond")],
            prefix1="",
            prefix2="",
            doc_string="",
            producer_name=var_pre.producer_name,
            producer_version=var_pre.producer_version,
            domain=var_pre.domain,
            model_version=var_pre.model_version,
        )
        var_pre_diffusion.graph.name = var_pre.graph.name
        var_predictor = onnx.compose.merge_models(
            var_pre_diffusion,
            var_post,
            io_map=[("xs_pred", "xs_pred")],
            prefix1="",
            prefix2="",
            doc_string="",
            producer_name=var_pre.producer_name,
