...
class DiffSingerVarianceInfer(BaseSVSInfer):
    def __init__(self, device=None, ckpt_steps=None, predictions: set = None):
        ...
        if hparams["use_spk_id"]:
            with open(
                pathlib.Path(hparams["work_dir"]) / "spk_map.json", "r", encoding="utf8"
            ) as f:
                self.spk_map = json.load(f)
            assert isinstance(self.spk_map, dict) and len(self.spk_map) > 0, (
                "Invalid or empty speaker map!"
            )
            assert len(self.spk_map) == len(set(self.spk_map.values())), (
        ...
        assert "none" not in glide_types, (
            "Type name 'none' is reserved and should not appear in glide_types."
        )
        self.glide_map = {
            "none": 0,
            **{typename: idx + 1 for idx, typename in enumerate(glide_types)},
        }
    ...
    def preprocess_input(
        self, param, idx=0, load_dur: bool = False, load_pitch: bool = False
    ):
        """
        ...
        if hparams.get("use_lang_id", False):
            languages = torch.LongTensor(
                [
                    (
                        self.lang_map[
                            lang if "/" not in p else p.split("/", maxsplit=1)[0]
                        ]
                        if self.phoneme_dictionary.is_cross_lingual(
                            p if "/" in p else f"{lang}/{p}"
                        )
                        else 0
                    )
        ...
        ph_num = torch.from_numpy(np.array([param["ph_num"].split()], np.int64)).to(
            self.device
        )  # [B=1, T_w]
        ...
        note_midi = np.array(
            [
                (librosa.note_to_midi(n, round_midi=False) if n != "rest" else -1)
                for n in param["note_seq"].split()
            ],
            dtype=np.float32,
        )
        ...
        if np.all(note_rest):
            ...
            note_midi = np.full_like(note_midi, fill_value=60.0)
        else:
            # Interpolate rest values
            interp_func = interpolate.interp1d(
                np.where(~note_rest)[0],
                note_midi[~note_rest],
                kind="nearest",
        ...
        note_dur_sec = torch.from_numpy(
            np.array([param["note_dur"].split()], np.float32)
        ).to(self.device)  # [B=1, T_n]
        note_acc = torch.round(
            torch.cumsum(note_dur_sec, dim=1) / self.timestep + 0.5
        ).long()
        ...
        if load_dur:
            ...
            ph_acc = torch.round(
                torch.cumsum(ph_dur_sec, dim=1) / self.timestep + 0.5
            ).long()
            ...
            word_dur = note_dur.new_zeros(1, T_w + 1).scatter_add(1, ph2word, ph_dur)[
                :, 1:
            ]  # => [B=1, T_w]
        else:
            ph_dur = None
            ...
            is_slur = torch.BoolTensor(
                [[int(s) for s in param["note_slur"].split()]]
            ).to(self.device)  # [B=1, T_n]
        ...
        if mel2word.shape[1] != T_s:  # Align words with notes
            mel2word = F.pad(
                mel2word, [0, T_s - mel2word.shape[1]], value=mel2word[0, -1]
            )
        ...
        if (
            hparams.get("use_glide_embed", False)
            and param.get("note_glide") is not None
        ):
            batch["note_glide"] = torch.LongTensor(
                [[self.glide_map.get(x, 0) for x in param["note_glide"].split()]]
            ).to(self.device)
        else:
            batch["note_glide"] = torch.zeros(
                1, T_n, dtype=torch.long, device=self.device
            )
        ...
        if ph_dur is not None:
            ...
            mel2pdur = torch.gather(
                F.pad(ph_dur, [1, 0], value=1), 1, mel2ph
        ...
        if self.model.predict_pitch:
            if load_pitch:
                ...
                # Load expressiveness
                expr = param.get("expr", 1.0)
                if isinstance(expr, (int, float, bool)):
                    summary["expr"] = f"static({expr:.3f})"
                    batch["expr"] = torch.FloatTensor([expr]).to(self.device)[
                        :, None
                    ]  # [B=1, T=1]
                else:
                    ...
                    batch["expr"] = torch.from_numpy(expr.astype(np.float32)).to(
                        self.device
                    )[None]
        ...
        if self.model.predict_variances:
            for v_name in self.model.variance_prediction_list:
                if (
                    self.auto_completion_mode
                    and param.get(v_name) is None
                    or v_name in self.variance_prediction_set
                ):
                    summary[v_name] = "auto"
                else:
        ...
        if self.use_key_shift_embed:
            shift_min, shift_max = hparams["augmentation_args"][
            ...
            if isinstance(gender, (int, float, bool)):
                ...
                key_shift_value = (
                    gender * shift_max if gender >= 0 else gender * abs(shift_min)
                )
                batch["key_shift"] = torch.FloatTensor([key_shift_value]).to(
                    self.device
                )[:, None]
            else:
                ...
                key_shift_seq = gender_seq * (
                    gender_mask * shift_max + (1 - gender_mask) * abs(shift_min)
                )
                batch["key_shift"] = torch.clip(
                    torch.from_numpy(key_shift_seq.astype(np.float32)).to(self.device)[
                        None
                    ],
                    min=shift_min,
                    max=shift_max,
        ...
        if self.use_speed_embed:
            if param.get("velocity") is None:
                ...
                batch["speed"] = torch.FloatTensor([1.0]).to(self.device)[:, None]
            else:
                summary["velocity"] = "manual"
                speed_min, speed_max = hparams["augmentation_args"][
                ...
                batch["speed"] = torch.clip(
                    torch.from_numpy(speed_seq.astype(np.float32)).to(self.device)[
                        None
                    ],
                    min=speed_min,
                    max=speed_max,
    ...
    def forward_model(self, sample):
        ...
        if hparams["use_spk_id"]:
            ...
            ph_spk_mix_embed = torch.sum(
                self.model.spk_embed(ph_spk_mix_id)
                * ph_spk_mix_value.unsqueeze(3),  # => [B, T_ph, N, H]
                dim=2,
                keepdim=False,
            )  # => [B, T_ph, H]
            spk_mix_embed = torch.sum(
                self.model.spk_embed(spk_mix_id)
                * spk_mix_value.unsqueeze(3),  # => [B, T_s, N, H]
                dim=2,
                keepdim=False,
            )  # [B, T_s, H]
        else:
        ...
        dur_pred, pitch_pred, variance_pred = self.model(
            ...
            pitch=pitch,
            pitch_expr=expr,
            ph_spk_mix_embed=ph_spk_mix_embed,
    ...
    def infer_once(self, param):
        ...
        if pitch_pred is not None:
            ...
            f0_pred = None
        variance_pred = {k: v[0].cpu().numpy() for k, v in variance_pred.items()}
    ...
    def run_inference(
        ...
        for i, param in enumerate(params):
            ...
            if self.auto_completion_mode:
                flag = (
                    ...
                    and any(
                        param.get(v_name) is None
                        for v_name in self.model.variance_prediction_list
                    ),
                )
            else:
                predict_variances = (
                    self.model.predict_variances and self.global_predict_variances
                )
                predict_pitch = self.model.predict_pitch and (
                    self.global_predict_pitch
                    or (param.get("f0_seq") is None and predict_variances)
                )
                predict_dur = self.model.predict_dur and (
                    self.global_predict_dur
                    or (
                        param.get("ph_dur") is None
                        and (predict_pitch or predict_variances)
                    )
                )
        ...
        for i in range(num_runs):
            ...
            for param, flag, batch in tqdm.tqdm(
                zip(params, predictor_flags, batches),
                desc="infer segments",
                total=len(params),
            ):
                if "seed" in param:
                ...
                if dur_pred is not None and (
                    self.auto_completion_mode or self.global_predict_dur
                ):
                    dur_pred = dur_pred[0].cpu().numpy()
                    param_copy["ph_dur"] = " ".join(
                        str(round(dur, 6))
                        for dur in (dur_pred * self.timestep).tolist()
                    )
                if pitch_pred is not None and (
                    self.auto_completion_mode or self.global_predict_pitch
                ):
                    pitch_pred = pitch_pred[0].cpu().numpy()
                    f0_pred = librosa.midi_to_hz(pitch_pred)
                    param_copy["f0_seq"] = " ".join(
                        [str(round(freq, 1)) for freq in f0_pred.tolist()]
                    )
                    param_copy["f0_timestep"] = str(self.timestep)
                variance_pred = {
                    k: v[0].cpu().numpy()
                    for k, v in variance_pred.items()
                    if (self.auto_completion_mode and param.get(k) is None)
                    or k in self.variance_prediction_set
                }
                for v_name, v_pred in variance_pred.items():
                    param_copy[v_name] = " ".join(
                        [str(round(v, 4)) for v in v_pred.tolist()]
                    )
